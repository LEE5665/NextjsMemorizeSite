...
export default function ShareLinkModal({ url, onClose, onCopy, copied }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(0,0,0,0.38)] modal-overlay-fade">
      <div
        ref={modalRef}
        className="w-full max-w-xs sm:max-w-md p-4 sm:p-7 mx-2 rounded-2xl shadow-2xl border bg-[var(--bg-color)] text-[var(--text-color)] border-[var(--border-color)] animate-modal-pop"
      >
        <div className="flex justify-between items-center mb-4 sm:mb-5">
          <h2 className="text-base sm:text-lg font-bold">공유 링크</h2>
        ...
        <div className="mb-4 sm:mb-5">
          <input
            ...
            readOnly
            value={url}
            className="w-full px-3 py-2 sm:py-3 rounded-xl bg-[var(--input-bg)] text-[var(--text-color)] border border-[var(--border-color)] font-mono text-xs sm:text-sm focus:outline-none select-all"
        ...
        <div className="flex justify-end">
          <button
            onClick={onCopy}
            className={`px-4 sm:px-5 py-2 rounded-lg font-bold shadow-sm transition
